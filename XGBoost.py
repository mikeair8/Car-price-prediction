...
class XGBM:
    def __init__(self, X, y, find_n=False, n_estimators=4500):
        self.X = X
        self.y = y
        self.find_n = find_n
    ...
    def print_metrics(y_true, y_pred, dataset_name='Validation'):
        ...
        mape = XGBM.mean_absolute_percentage_error(y_true, y_pred)
    ...
    def find_best_n_estimators(self):
        ...
        best_n_estimators = 0
        for n_estimators in range(500, 7000, 500):
            xgbm = XGBRegressor(n_estimators=n_estimators, learning_rate=0.1, max_depth=3, random_state=42,
                                objective='reg:squarederror')
            xgbm.fit(X_train, y_train)
            y_val_pred = xgbm.predict(X_val)
    ...
    def train_and_evaluate(self, use_saved_model=False):
        ...
        if use_saved_model and os.path.exists('best_model_xgbm.pkl'):
            print("Loading saved model...")
            self.model = joblib.load('best_model_xgbm.pkl')
        else:
            print("Training new model...")
            self.model = XGBRegressor(n_estimators=self.n_estimators, learning_rate=0.1, max_depth=3, random_state=42,
                                      objective='reg:squarederror')
            self.model.fit(X_train_full, y_train_full)
            joblib.dump(self.model, 'best_model_xgbm.pkl')  # Save the model
        ...
        for name, (X_set, y_set) in datasets.items():
            y_pred = self.model.predict(X_set)
            xgbm_metrics[name] = self.get_metrics(np.exp(y_set), np.exp(y_pred))
        ...
        #test_predictions.to_csv('predicted_vs_actual_XGBM.csv', index=False)
        print(test_predictions.head())
        return xgbm_metrics, test_predictions
